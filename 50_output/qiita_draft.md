# リクエスト課金のLLM APIで最適バッチサイズを実測した — 限界を決めるのは「モデルの賢さ」ではなく「出力トークン予算」だった

## TL;DR

- さくらのAI Engine の無償枠は**トークン数ではなくリクエスト数**で数える（Chat completions 月3,000回）。この前提では「1件ずつ投げる」設計が極端に非効率になる
- 96件を1リクエストにまとめても、**正解率100%・パース失敗ゼロ**。実効コストは8件ずつ投げた場合の**12分の1**になった
...
- **28.8万件という数字は外挿**です。実測したのは192件分の処理です
