...
LLM の API で大量のデータを処理するとき、私はいつも1件ずつループで投げていました。
...
トークン数で課金される API なら、これで何も問題ありません。100件を1回にまとめて送っても100回に分けて送っても、トークンの合計はほとんど変わらないので**料金が同じ**です。それなら、出力が単純で壊れにくく、失敗しても1件だけやり直せばいい「1件ずつ」のほうが安全でした。

ところが、さくらの AI Engine の無償枠は**リクエストの回数**で数えます。月3,000回まで無料で、そのなかでトークンをどれだけ使っても同じ扱いです。この前提だと「1件ずつ」では月3,000件で打ち止めになってしまいます。

そこで思いつくのが**バッチング**、つまり複数の件数を1回のリクエストにまとめて処理することです。96件を1回で送れれば、同じ枠で96倍処理できる計算になります。

とはいえ、詰め込みすぎれば出力が壊れるはずです。**では、その境界はどこにあるのか。** 合計740リクエストを使って実測してみました。

リクエスト数で上限のある API を使っている方や、レート制限に張り付いて困っている方の参考になれば嬉しいです。

## 先に結論

長い記事なので、先に分かったことをまとめます。

**① 96件を1リクエストにまとめても、まったく壊れませんでした。**
ラベルだけ返す軽いタスクなら、96件を1回で送って正解率100%。8件ずつ送る場合と比べて、1件処理するのに必要なリクエスト数は**12分の1**になりました。

**② 壊れる原因は「モデルが混乱すること」ではなく、出力の文字数制限でした。**
出力を重くしていくと崩壊しますが、原因はすべて `max_tokens`（1回の応答で生成できる上限）に達して途中で打ち切られたことでした。**JSON の構造が壊れたケースは1件もありません。**

**③ 必要な上限値は、バッチサイズにほとんど関係ありませんでした。**
ここが一番意外でした。8件しか送っていないのに11,669トークン使ったケースがあります。出力する本文の量ではなく、**モデルが考える時間（思考トークン）のばらつき**が上限を決めていたためです。

**④ したがって、手順はシンプルになります。**
まず `max_tokens` を最悪ケースに合わせて大きく取る。そのうえでバッチを大きくする。出力は JSONL 形式（1行1件）にする。この順番なら、リクエスト課金では効率が桁で変わります。

![実験結果の全体像](../40_test/analysis/figures/fig1_recovery_by_batchsize.png)

図は、出力の重さを3段階に変えて、バッチサイズごとに「送った件数のうち何件を回収できたか」を見たものです。左は無傷、右へいくほど崩壊が起きています。以降で1つずつ見ていきます。
...
トークン課金なら「1件ずつ」に何のデメリットもありませんでした。壊れにくく、失敗時のリトライも1件分で済み、並列化もしやすいからです。ところがリクエスト課金では、その安全策が**そのまま処理能力の天井**になってしまいます。

とはいえ、バッチを大きくすれば出力が壊れやすくなるはずです。
そこで、実際にさくらのAI Engineを使用し、**どこに損益分岐点があるのか**を実測してみました。
